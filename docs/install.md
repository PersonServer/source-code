...
- **Let long polls finish — at every hop.** Agents poll pending requests
  with `Prefer: wait=N`; psd holds the connection up to 50 seconds. Any
  proxy in front of psd with a request/read timeout below that turns every
  long poll into a `504` from *the proxy* — the agent never sees psd's
  answer, and the approval it was waiting for is delivered to nobody. Set
  the read/request timeout to **at least 60 seconds on each hop**: nginx
  `proxy_read_timeout 60s;`; ingress-nginx
  `nginx.ingress.kubernetes.io/proxy-read-timeout: "60"` (the chart's
  default annotation); Gateway API `HTTPRoute.spec.rules[].timeouts.request`
  and `.backendRequest: 75s` — Envoy-based gateways default to **15 s**, and
  that is the one people actually hit; APISIX upstream `read`/`send`
  timeouts likewise. Found live on a sandbox behind APISIX → Envoy Gateway
  → psd, where both walls had to move.
